...
import org.klose.payment.server.integration.alipay.bean.AlipayOrderVO;
...
public class AlipayPaymentService implements EbaoPaymentService {
	...
	protected AlipayOrderVO convert(BillingData data, String partner) {
		...
		vo.setContextUrl(data.getHostEndpoint());
	...
	private PaymentForm generateForwardViewData(AlipayOrderVO orderVO,
		...
		if (config.isWapPayment()) {
			...
			// params.put("seller_email", config.getSellerEmail());
